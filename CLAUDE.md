# CLAUDE.md — Mere Repository

This file currently covers workspace tooling only. See `design_docs/` for
project context and architecture
(`design_docs/2026-05-24_external_deps_topology_brief.md` has the deep
reference on the tooling below, under "Workspace tooling: sem & weave").
...
editing it, especially across the sibling-repo lattice (mere is the
supercrate most other repos consume). Avoid unfiltered scans over large
directories: `sem entities crates --json` on a Mere-sized tree produces a
very large stream.
