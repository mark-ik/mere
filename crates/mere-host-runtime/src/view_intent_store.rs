...
pub fn load_view_intent(
    ...
    let intent: ViewIntent = serde_json::from_str(&text)
        .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?;
    Ok(Some(intent))
}
...
mod tests {
    ...
    fn temp_session_dir(label: &str) -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!(
            "mere-view-intent-test-{label}-{pid}-{nanos}"
        ));
    ...
    fn save_overwrites_existing_intent_atomically() {
        ...
        assert!(restored.hidden_relations.iter().any(|r| r.from_id == Uuid::from_u128(0xc1)));
