...
impl ManifestStore {
    ...
    pub fn update(
        &mut self,
        id: SessionId,
        f: impl FnOnce(&mut GraphSessionManifest),
    ) -> bool {
    ...
    pub fn load_from_disk(&mut self, root: impl Into<PathBuf>) -> io::Result<LoadReport> {
        ...
        for entry in fs::read_dir(&root)? {
            ...
            match fs::read_to_string(&manifest_path)
                ...
                .and_then(|text| {
                    serde_json::from_str::<GraphSessionManifest>(&text)
                        .map_err(|e| e.to_string())
                }) {
                Ok(manifest) => {
                    let id = manifest.session_id;
...
mod tests {
    ...
    fn fixture_session(idx: u128) -> (SessionId, GraphId) {
        (
            SessionId::from_uuid(Uuid::from_u128(0x1000_0000_0000_0000_0000_0000_0000_0000 + idx)),
            GraphId::from_uuid(Uuid::from_u128(0x2000_0000_0000_0000_0000_0000_0000_0000 + idx)),
        )
    }
    ...
    fn temp_root(label: &str) -> PathBuf {
        ...
        let root = std::env::temp_dir().join(format!(
            "mere-host-runtime-test-{label}-{pid}-{nanos}"
        ));
