...
pub fn build_switcher_thumbnail(
    ...
    let single_node_position = if kept.len() == 1 || (max_x - min_x).abs() < f32::EPSILON
        && (max_y - min_y).abs() < f32::EPSILON
    {
...
mod tests {
    ...
    fn hyperlink_edge_yields_family_tag_for_semantic() {
        ...
        // maps consistently.
        assert_eq!(t.edges[0].family_tag, expected_family_tag(EdgeFamily::Semantic));
    }
