//! `verso` — the compatibility-view flip orchestrator (the carrier).
//!
...
//! This crate is the engine-agnostic carrier: it pairs a [`FlipDonor`] with a
...
//! the receiver the result. It owns no engine and no GPU layer (it depends only on
//! `verso-api`), so a heavy engine only ever lands in the host variant that asked
//! for it, never here.
//!
...
mod tests {
    use super::*;
    use verso_api::{Cookie, FormValues};
