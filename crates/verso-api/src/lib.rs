//! `verso-api` — the engine-flip seam of the compatibility-view charter.
//!
...
//! This crate is the engine-agnostic contract: the portable view-state moved across
//! a flip, plus the donor / back / receiver traits. It depends on no engine and no
//! GPU layer. Per-engine adapters (`verso-serval`, `verso-scry`, ...) bridge concrete
//! engines to these traits; the `verso` orchestrator pairs a donor with a receiver
//! and runs the flip choreography.
