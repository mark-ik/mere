//! `verso-serval` — the serval [`FlipDonor`] adapter for the compatibility-view flip.
//!
...
//! engine APIs to [`verso_api::PortableViewState`]. It reaches three sources, by
//! layer:
//!
...
//! `with_*` setters. That keeps this crate depending only on `serval-scripted-dom`
//! and `verso-api` (never the runtime or the GPU layer), and mirrors how the
//! `verso-scry` receiver is host-fed. The `verso` orchestrator builds a donor at
//! flip time, calls [`capture`](FlipDonor::capture) once, and drops it.

use layout_dom_api::LayoutDom;
use serval_scripted_dom::ScriptedDom;
use verso_api::{Cookie, FlipDonor, FormValues, FrameHandle, LayerSet, PortableViewState};
