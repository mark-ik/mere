...
pub type ProducerFactory =
    Arc<dyn Fn(&SceneNodeRef) -> Box<dyn WebSurfaceProducer> + Send + Sync>;
...
mod tests {
    ...
    use scrying::native_frame::{
        CapabilityStatus, HostWgpuContext, NativeFrameKind, UnsupportedReason,
        WgpuTextureImporter,
    };
    use scrying::{
    ...
    fn ensure_producer_creates_state_and_release_clears_it() {
        ...
        let mut renderer =
            ScryingEmbeddedRenderer::new("test.scrying.web", NodeContentKind::WebPage, importer, factory);
