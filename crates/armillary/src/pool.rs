...
//!
//! Why this exists: some actors are long-lived (a per-tile content actor lives as
//! long as its tab) and `thread::spawn`-per-actor leaks one OS thread's worth of
//! per-thread state on every spawn — notably Stylo's *leaked* thread-local sharing
//! cache, which is never reclaimed. Opening / closing / respawning tiles would then
//! leak unboundedly with churn.
//!
