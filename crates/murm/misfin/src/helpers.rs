...
pub(super) fn parse_sender_line(line: &str) -> Option<MisfinSender> {
...
pub(super) fn parse_recipients_line(line: &str) -> Option<Vec<MisfinAddress>> {
    let remainder = line.strip_prefix(':')?.trim();
...
pub(super) fn parse_timestamp_line(line: &str) -> Option<String> {
    let remainder = line.strip_prefix('@')?.trim();
    if remainder.is_empty() {
        None
    } else {
