...
fn cluster_by_affinity_installs_and_clears_the_affinity_force() {
    ...
    let _ = orrery.frame(800, 600);
    assert_eq!(orrery.affinity_pair_count(), 0, "off => the force is cleared");
}
...
fn affinity_force_refreshes_on_a_topology_change() {
    ...
    orrery.set_cluster_by_affinity(true);
    let _ = orrery.frame(800, 600);
    assert_eq!(orrery.affinity_pair_count(), 0, "an edgeless graph has no affinity pairs");
...
fn gloss_lens_assembles_and_caches_independent_positions() {
    ...
    assert!(!orrery.gloss_needs_recompute(200, 200), "mirroring needs no recompute");
    ...
    assert!(orrery.gloss_needs_recompute(200, 200), "a fresh lens needs computing");
    orrery.set_gloss_positions(
        vec![(ak, PortablePoint::new(10.0, 20.0)), (bk, PortablePoint::new(30.0, 40.0))],
        Vec::new(),
        200,
        200,
    );
    assert!(!orrery.gloss_needs_recompute(200, 200), "unchanged inputs => cached");
    ...
    let a_node = nodes.iter().find(|(id, _, _, _)| *id == a_id).expect("node a is in the gloss");
    assert_eq!(a_node.1, (10.0, 20.0), "the gloss uses the lens position, not the live layout");

    // A viewport change re-triggers; so does a topology change.
    assert!(orrery.gloss_needs_recompute(300, 200), "a viewport change re-triggers");
    orrery.ingest_graph(|g| {
        g.add_node("https://c.example".to_string(), PortablePoint::new(2.0, 0.0));
        true
    });
    assert!(orrery.gloss_needs_recompute(200, 200), "a topology change re-triggers");
}
...
fn gloss_overlays_resolve_into_rings_at_lens_positions() {
    ...
    let overlays = vec![
        ...
        signals::Overlay::BridgeEmphasis { node: ak, weight: 1.0 },
    ];
    orrery.set_gloss_positions(
        vec![(ak, PortablePoint::new(10.0, 20.0)), (bk, PortablePoint::new(30.0, 40.0))],
        overlays,
...
fn gloss_scope_selection_crops_the_lens_to_the_selection() {
    ...
    let ak = orrery.graph().get_node_by_url("https://a.example").unwrap().0;
    let bk = orrery.graph().get_node_by_url("https://b.example").unwrap().0;
    let ck = orrery.graph().get_node_by_url("https://c.example").unwrap().0;
    ...
    assert_eq!(edges.len(), 0, "no induced edge (a's neighbour b is out of scope)");
...
fn gloss_scope_is_part_of_the_lens_cache_key() {
    ...
    let mut graph = Graph::new();
    let a = graph.add_node("https://a.example".to_string(), PortablePoint::new(0.0, 0.0));
    graph.add_node("https://b.example".to_string(), PortablePoint::new(1.0, 0.0));
    let mut orrery = Orrery::with_graph(graph);
    let ak = orrery.graph().get_node_by_url("https://a.example").unwrap().0;

    orrery.set_gloss_strategy(Some("spectral.default".to_string()));
    assert_eq!(orrery.gloss_scope_keys(), None, "no scope until selection-scoping is on");
    orrery.set_gloss_positions(vec![(ak, PortablePoint::new(0.0, 0.0))], Vec::new(), 200, 200);
    assert!(!orrery.gloss_needs_recompute(200, 200), "unchanged inputs => cached");

    // Selecting + scoping changes the scope key, so the lens must recompute (subgraph re-layout).
    orrery.select_by_url("https://a.example");
    orrery.set_gloss_scope_selection(true);
    assert_eq!(orrery.gloss_scope_keys(), Some(vec![ak]), "the scope is the sorted selection");
    assert!(orrery.gloss_needs_recompute(200, 200), "a scope change re-triggers the lens");
}
...
fn gloss_edges_carry_multiplicity_weight() {
    ...
    assert!(weights[1] >= 2.0, "the double-relation pair weighs at least 2, got {}", weights[1]);
}
...
fn gloss_size_by_importance_scales_nodes_by_the_signal() {
    ...
    let mut graph = Graph::new();
    let hub = graph.add_node("https://hub.example".to_string(), PortablePoint::new(0.0, 0.0));
    let l1 = graph.add_node("https://l1.example".to_string(), PortablePoint::new(1.0, 0.0));
    let l2 = graph.add_node("https://l2.example".to_string(), PortablePoint::new(2.0, 0.0));
    graph.assert_semantic_predicate(hub, l1, "links".to_string());
    graph.assert_semantic_predicate(hub, l2, "links".to_string());
    let mut orrery = Orrery::with_graph(graph);
    let hubk = orrery.graph().get_node_by_url("https://hub.example").unwrap().0;
    let l1k = orrery.graph().get_node_by_url("https://l1.example").unwrap().0;
    let l2k = orrery.graph().get_node_by_url("https://l2.example").unwrap().0;
...
fn gloss_kanban_by_site_lens_always_recomputes() {
    ...
    let mut graph = Graph::new();
    graph.add_node("https://a.example".to_string(), PortablePoint::new(0.0, 0.0));
    let mut orrery = Orrery::with_graph(graph);
    let ak = orrery.graph().get_node_by_url("https://a.example").unwrap().0;
