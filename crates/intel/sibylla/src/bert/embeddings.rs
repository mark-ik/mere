...
impl<B: Backend> BertEmbeddings<B> {
    ...
    pub fn from_loaded(
        config: &BertConfig,
        loaded: &super::loaded::LoadedEmbeddings<B>,
        device: &B::Device,
    ) -> Self {
        Self {
    ...
    pub fn forward(&self, input_ids: Tensor<B, 2, Int>) -> Tensor<B, 3> {
        ...
        let position_ids: Tensor<B, 2, Int> = Tensor::arange(0..seq_len as i64, &device)
            .reshape([1, seq_len])
            .repeat_dim(0, batch_size);

        // Single-segment input → all token type ids = 0.
        let token_type_ids: Tensor<B, 2, Int> = Tensor::zeros([batch_size, seq_len], &device);
...
mod tests {
    ...
    fn embeddings_construct_without_panic() {
        ...
        let _emb: BertEmbeddings<B> = BertEmbeddings::new(&config(), &device);
    }

    #[test]
    fn forward_returns_correct_shape() {
        let device = Default::default();
        let emb: BertEmbeddings<B> = BertEmbeddings::new(&config(), &device);
        // batch=2, seq_len=5
    ...
    fn forward_handles_single_token() {
        let device = Default::default();
        let emb: BertEmbeddings<B> = BertEmbeddings::new(&config(), &device);
        let input_ids: Tensor<B, 2, Int> = Tensor::from_data([[101]], &device);
        let out = emb.forward(input_ids);
