...
/// dtype and shape, and constructing a Burn `Tensor<B, D>` on the given
/// device.
pub fn extract<B: Backend, const D: usize>(
    view: &TensorView<'_>,
    expected_shape: [usize; D],
    device: &B::Device,
) -> Result<Tensor<B, D>, LoaderError> {
...
pub fn extract_1d<B: Backend>(
    ...
    expected_len: usize,
    device: &B::Device,
) -> Result<Tensor<B, 1>, LoaderError> {
    extract::<B, 1>(view, [expected_len], device)
}
...
pub fn extract_2d<B: Backend>(
    ...
    expected_cols: usize,
    device: &B::Device,
) -> Result<Tensor<B, 2>, LoaderError> {
    extract::<B, 2>(view, [expected_rows, expected_cols], device)
}
...
mod tests {
    ...
    fn extract_2d_round_trips_f32() {
        ...
        let t: Tensor<B, 2> = extract_2d(&view, 2, 3, &device).unwrap();
        let out = t.into_data().to_vec::<f32>().unwrap();
        assert_eq!(out, values);
    }
    ...
    fn rank_mismatch_errors() {
        ...
        let result = extract::<B, 2>(&view, [1, 2], &device);
    ...
    fn shape_mismatch_errors() {
        ...
        let result = extract_1d::<B>(&view, 4, &device);
        assert!(matches!(result, Err(LoaderError::InvalidWeights(_))));
    }
    ...
    fn extract_2d_with_realistic_dimensions() {
        ...
        let t: Tensor<B, 2> = extract_2d(&view, 16, 8, &device).unwrap();
