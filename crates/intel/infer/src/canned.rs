...
//! Deterministic stub provider — `embed::hashed`'s sibling for inference.
//!
//! Exact-match prompts return their canned response; anything else echoes
//! the prompt (`echo: <prompt>`). Output streams word by word (with
//! leading spaces on continuation fragments), honouring `max_tokens` and
//! `stop`, so callers exercise real streaming/truncation logic without a
//! model, a GPU, or nondeterminism.
...
/// Deterministic canned/echo inference provider for tests and dev harnesses.
#[derive(Debug, Clone)]
...
impl CannedProvider {
    ...
    pub fn new() -> Self {
        Self {
            capability: ModelCapability {
                model_id: "canned/echo".to_string(),
                context_window: 4096,
                quantization: None,
                loader: "canned".to_string(),
                streaming: true,
            },
    ...
    /// Add an exact-match canned response.
...
mod tests {
    ...
    #[test]
    fn canned_response_wins_over_echo() {
    ...
    fn stop_sequence_halts_before_emitting() {
        let p = CannedProvider::new().with_response("q", "alpha beta STOP gamma");
        let mut fragments = Vec::new();
        let out = p
            .generate_streaming(
                &GenerationRequest {
    ...
    fn prompt_too_long_reports_window() {
        let p = CannedProvider::new().with_context_window(3);
        let err = p
            .generate(&GenerationRequest {
    ...
    fn provider_is_object_safe_and_shared() {
        let p: Box<dyn InferenceProvider> = Box::new(CannedProvider::new());
        assert_eq!(p.capability().loader, "canned");
