...
/// What a loaded model/runtime pair can do — the matching surface a caller
/// uses to pick a provider, and the status surface Steward-style UI shows.
///
/// This is deliberately small: the fields the local-models harness brief
/// names (context window, quantization, runtime-loader id) plus identity
/// and the streaming flag. The LoRA-adapter compatibility envelope is a
/// separate, richer contract (geist §5) that rides the future
/// `AdapterLoader` seam, not this descriptor.
...
pub struct ModelCapability {
    ...
    /// (e.g. `canned`, `burn-wgpu`, `mistral.rs`).
    pub loader: String,
