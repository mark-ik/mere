...
//!
//! This slice computes full-sequence forwards (prefill shape). The KV
//! cache enters with the generation-loop slice, via burn-nn's
...
impl<B: Backend> DecoderAttention<B> {
    ...
    /// Full-sequence causal forward. `x: [batch, seq, hidden]`; `rope` is
    /// the model-owned rotary table (one per model, shared by all layers);
    /// `start` is the absolute position of `x`'s first token.
    pub fn forward(&self, x: Tensor<B, 3>, rope: &RotaryEncoding<B>, start: usize) -> Tensor<B, 3> {
        ...
        // Rotate q and k by absolute position.
        ...
        let expand_kv = |t: Tensor<B, 4>| -> Tensor<B, 4> {
            ...
            t.reshape([batch, self.n_kv_heads, 1, seq * self.head_dim])
                .repeat_dim(2, group)
        ...
        let mask = generate_autoregressive_mask::<B>(batch, seq, &device)
            .unsqueeze_dim::<4>(1)
            .expand([batch, self.n_heads, seq, seq]);
