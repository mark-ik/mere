...
//! payoff receipts). All `#[ignore]`d — they need `MERE_TINYLLAMA_DIR`
//! pointing at a directory with TinyLlama-1.1B-Chat-v1.0's `config.json`
//! / `tokenizer.json` / `model.safetensors` (HF layout, bf16). Run:
//!
//! ```bash
//! MERE_TINYLLAMA_DIR=C:/t/models/TinyLlama-1.1B-Chat-v1.0 \
//!     cargo test -p infer --features decoder-wgpu --release \
...
use infer::decoder::DecoderProvider;
use infer::{GenerationRequest, InferenceProvider};

type Cpu = burn::backend::NdArray<f32>;

fn model_dir() -> Option<PathBuf> {
    std::env::var("MERE_TINYLLAMA_DIR").ok().map(PathBuf::from)
}

fn load_provider<B: Backend>(loader: &str) -> DecoderProvider<B> {
    let dir = model_dir().expect("MERE_TINYLLAMA_DIR must be set");
...
/// "Paris".
#[test]
#[ignore = "requires MERE_TINYLLAMA_DIR (2.2GB checkpoint)"]
...
#[test]
#[ignore = "requires MERE_TINYLLAMA_DIR (2.2GB checkpoint) and a GPU"]
