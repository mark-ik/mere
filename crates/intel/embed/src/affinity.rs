...
//! Bridge B — embedding similarity as a **clustering affinity signal**
//! (burn brief Lane 5, P4).
//!
//! Where [`field_bridge`](crate::field_bridge) renders a *query*'s similarity
//! as a scalar field over canvas space, this renders *node-to-node* similarity
//! as a sparse list of `(a, b, weight)` affinity pairs: each node's top-K
//! nearest neighbours above a threshold, weighted by cosine similarity. That is
//! exactly the shape gyre's `AffinitySpring` consumes (structural Jaccard was
//! the placeholder; content-embedding cosine is the intended signal), so a graph
//! whose nodes carry embeddings clusters by meaning under the force-directed
//! layout — semantically-similar nodes draw together even with no direct edge.
//!
//! Pure over a [`VectorIndex`]; no gyre dependency. The host maps the returned
//! `K` to a `NodeKey` and builds the `AffinitySpring`.
