...
//! the model happens via [`crate::bert::loader::load_into_model`].
//!
...
mod tests {
    ...
    fn synthesized_loaded_sentence_forward_returns_normalized_vec() {
        ...
        let model = loaded.into_model(&device);
        let input_ids: Tensor<B, 2, Int> = Tensor::from_data([[1, 2, 3, 4, 5]], &device);
        let out = model.forward_sentence(input_ids, crate::bert::model::Pooling::Mean, true);
    ...
    fn synthesized_loaded_is_deterministic_per_input() {
        ...
        let a = model
            .forward_sentence(input_ids.clone(), crate::bert::model::Pooling::Mean, true)
            .into_data()
            .to_vec::<f32>()
            .unwrap();
        let b = model
            .forward_sentence(input_ids, crate::bert::model::Pooling::Mean, true)
            .into_data()
            .to_vec::<f32>()
            .unwrap();
