...
pub(crate) fn rms_norm_from_loaded<B: Backend>(
    gamma: Tensor<B, 1>,
    epsilon: f64,
    device: &B::Device,
) -> RmsNorm<B> {
...
pub(crate) fn swiglu_from_loaded<B: Backend>(
    gate_w: Tensor<B, 2>,
    up_w: Tensor<B, 2>,
    device: &B::Device,
) -> SwiGlu<B> {
...
pub struct LoadedDecoderLayer<B: Backend> {
    pub input_norm_gamma: Tensor<B, 1>,
    pub q_w: Tensor<B, 2>,
    pub k_w: Tensor<B, 2>,
    pub v_w: Tensor<B, 2>,
    pub o_w: Tensor<B, 2>,
    pub post_norm_gamma: Tensor<B, 1>,
    pub gate_w: Tensor<B, 2>,
    pub up_w: Tensor<B, 2>,
    pub down_w: Tensor<B, 2>,
}

impl<B: Backend> DecoderLayer<B> {
    pub fn from_loaded(
        config: &DecoderConfig,
        loaded: LoadedDecoderLayer<B>,
        device: &B::Device,
    ) -> Self {
        Self {
...
mod tests {
    ...
    fn layer_forward_shape_and_finite_and_deterministic() {
        ...
        let rope = RotaryEncodingConfig::new(config.max_position_embeddings, config.head_dim())
            ...
            .init(&dev);
        let x = t2::<B>(5, config.hidden_size, 99, &dev).reshape([1, 5, config.hidden_size]);
...
mod tests_wgpu {
    ...
    fn layer_out<B: Backend>(config: &DecoderConfig) -> Vec<f32> {
        let dev = <B as BackendTypes>::Device::default();
        let h = config.hidden_size;
        let kv = config.kv_heads() * config.head_dim();
        let inter = config.intermediate_size;
        let t2 = |a: usize, b: usize, salt: usize| -> Tensor<B, 2> {
    ...
    fn layer_parity_ndarray_wgpu() {
        let config = tiny_config();
        let cpu = layer_out::<NdArray<f32>>(&config);
        let gpu = layer_out::<Wgpu<f32, i32>>(&config);
        let max_diff = cpu
            .iter()
            .zip(&gpu)
            .map(|(a, b)| (a - b).abs())
