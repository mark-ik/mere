    ...
    pub(crate) fn on_mouse_release(&mut self, button: MouseButton) {
        ...
        if let Some(drag) = self.view.tear_out_drag.take() {
            ...
            if moved {
                ...
                let cmd = match dest {
                    ...
                    Some(to) if self.view.modifiers.alt => crate::ShellCommand::MoveNodeAcross {
                        node: drag.node,
                    ...
                    None => match drag.op {
                        crate::window_view::TearOp::Fork => crate::ShellCommand::ForkNode {
                            node: drag.node,
                            from: drag.source_graph,
                        },
                        crate::window_view::TearOp::Branch => crate::ShellCommand::BranchNode {
                            node: drag.node,
                            from: drag.source_graph,
                        },
                    },
        ...
        if button == MouseButton::Left && !released_in_workbench {
            ...
            let double = self.view.last_left_release.is_some_and(|(t, (lx, ly))| {
                now.duration_since(t) < Duration::from_millis(400) && (x - lx).hypot(y - ly) < 6.0
            });
