...
pub enum ContentCommand {
    ...
    Resize { viewport: (u32, u32), viewport_gen: ViewportGeneration },
    ...
    spawn(wake, |commands, out: Emitter<ContentUpdate>| {
        ...
        while let Ok(command) = commands.recv() {
            match command {
                ...
                ContentCommand::Resize { viewport, viewport_gen } => {
...
mod tests {
    ...
    fn show_renders_a_scene_off_thread() {
        ...
        assert!(glyph_runs(&scene) >= 1, "the off-thread render lowered text to glyph runs");
    }
    ...
    fn show_harvests_embedded_jsonld_into_a_contribution() {
        ...
        let harvested = updates.iter().any(|u| match u {
            ContentUpdate::Contribution { contributions } => {
                contributions.iter().any(|c| c.nodes.iter().any(|n| n.id == "mere://z"))
            },
            _ => false,
        });
