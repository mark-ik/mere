...
impl WindowCtx<'_> {
    ...
    pub(crate) fn orrery_settings_items(&self) -> Vec<PaneItem> {
        ...
        if sbi {
            let by_degree = self.orrery().importance_metric() == mere::canvas::ImportanceMetric::Degree;
            items.push(pick(
        ...
        if bridges {
            let by_between = self.orrery().bridge_metric() == mere::canvas::BridgeMetric::Betweenness;
            items.push(pick(
