    ...
    fn run_crawl_honors_robots_disallow() {
        ...
        let site: HashMap<String, &str> = [
            ("https://s.test/robots.txt".to_string(), "User-agent: *\nDisallow: /private"),
            ("https://s.test/".to_string(), "<a href='/ok'>ok</a><a href='/private/secret'>x</a>"),
            ("https://s.test/ok".to_string(), "<p>ok</p>"),
            ("https://s.test/private/secret".to_string(), "<p>should be skipped</p>"),
        ]
        .into_iter()
        .collect();
        let fetch = move |url: String| {
        ...
        assert!(visited.contains(&"https://s.test/".to_string()), "seed fetched");
        assert!(visited.contains(&"https://s.test/ok".to_string()), "allowed path fetched");
        assert!(
