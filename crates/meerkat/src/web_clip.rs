//! Web clip command support: capture a semantic fragment from a live surface or fall back to
//! clipping the focused document body, then write it as a local knot node with provenance.
...
use super::{WindowCtx, fetch, render};

#[derive(Clone, Debug, PartialEq)]
pub(crate) struct ClipFragment {
    ...
    pub(crate) rect: Option<ClipRect>,
    pub(crate) visual: Option<ClipVisual>,
...
impl WindowCtx<'_> {
    ...
    fn create_clip_node(&mut self, source_member: GraphMemberId, fragment: ClipFragment) -> String {
        ...
        let mut write = ClipWriteResult { member: None };
        self.orrery_mut().ingest_graph(|graph| {
            write = write_clip_node(graph, source_key, &clip_url, &fragment);
            write.member.is_some()
        });

        let Some(member) = write.member else {
            return "clip: source node is missing".to_string();
        };
...
fn write_clip_node(
    ...
    ClipWriteResult { member }
}

pub(crate) fn web_clip_script(x: i32, y: i32) -> String {
    format!(
        r##"(function() {{
  const rawX = {x};
  const rawY = {y};
...
pub(crate) fn parse_web_clip(raw: &str, source_url: &str) -> Result<ClipFragment, String> {
...
pub(crate) fn attach_cropped_visual(
    ...
    let data_uri = format!(
        ...
        render::base64_encode(&png_bytes)
    );
...
fn clip_title(fragment: &ClipFragment) -> String {
    let base = fragment
        .title
        .as_deref()
        .filter(|title| !title.trim().is_empty())
    ...
    fn write_clip_node_records_graph_relation_and_thumbnail() {
        ...
        let clip = ClipFragment {
            ...
            visual: Some(ClipVisual {
                ...
                width: 2,
                height: 2,
                data_uri: format!("data:image/png;base64,{}", render::base64_encode(&png)),
            }),
            blocks: None,
        };

        let result = write_clip_node(&mut graph, source, "knot://clip/test", &clip);
        let member = result.member.expect("clip member");
