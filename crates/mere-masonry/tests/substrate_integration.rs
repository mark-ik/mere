...
fn substrate_host_dispatches_embedded_frame_through_full_chain() {
    ...
    assert_eq!(compositor.len(), 1, "compositor registered the panel texture");
    ...
    assert_eq!(compositor.len(), 1, "no duplicate registration on second render");
    ...
    assert_eq!(compositor.len(), 0, "compositor cleaned up after release_node");
    assert!(!compositor.is_registered(node_id));
}
