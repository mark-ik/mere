...
Implements `eidetic::BlobFetcher::fetch` for `BlobSource::Iroh { ticket }` by parsing the ticket as `"<node-id-hex>/<blob-hash-hex>"`, fetching the blob from the named peer through [`mere-transport`]'s iroh-blobs integration, and returning the bytes.
