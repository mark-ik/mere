...
Headless and app-agnostic. Depends on `chartulary` for the nested-graph
substrate and on `personae` (imported as `identity`) for the signed delegation
grammar.
...
| `cap` | `Cap`, `Capability`, `ScopePath`, `CapError`, `assert_capability_laws` |
...
segment, then commit through chartulary's attributed revision-checked batch. It
returns `Committed` or a `GateError` of `Unauthorized`, `OutOfScope`,
`TouchesProjection`, or `Commit`.
