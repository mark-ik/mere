...
if (-not $Out) {
    ...
    $Out = Join-Path $codeRoot "testing\mere\w3a-windows-maximized-$stamp"
}
...
$scenario = Join-Path $repoRoot 'crates\cambium\cambium-genet-winit-host\examples\smoke_windows_maximized.scn'
...
[ordered]@{
    repo = 'genet'
    scenario = 'crates/cambium/cambium-genet-winit-host/examples/smoke_windows_maximized.scn'
    platform = 'windows'
    commit = $commit
    dirty = $dirty
