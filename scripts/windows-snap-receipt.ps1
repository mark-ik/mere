...
if (-not $Out) {
    ...
    $Out = Join-Path $codeRoot "testing\mere\w4-windows-snap-$stamp"
}
...
$scenario = Join-Path $repoRoot 'crates\cambium\cambium-genet-winit-host\examples\smoke_windows_snap.scn'
...
[ordered]@{
    repo = 'genet'
    scenario = 'crates/cambium/cambium-genet-winit-host/examples/smoke_windows_snap.scn'
    platform = 'windows'
    commit = $commit
