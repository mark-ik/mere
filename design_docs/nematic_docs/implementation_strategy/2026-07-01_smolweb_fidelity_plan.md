...
| --- | --- | --- |
| RSS/Atom | `<summary>` (abstract) vs `<content>` (full body) | merged into one `summary` ([feed.rs:212](../../../../errand/src/parse/feed.rs)); the article body is dropped |
| Atom | `published` vs `updated`; RSS `pubDate` | merged into one `date` ([feed.rs:207](../../../../errand/src/parse/feed.rs)) |
...
| Atom | multiple `<link rel="alternate\|self\|enclosure">` | first-wins, `rel` ignored ([feed.rs:174](../../../../errand/src/parse/feed.rs)) |
| Gopher | item-type family (RFC 1436 + gopher+) | `g`/`I`→Image, `9`→Binary, but `4`/`5`/`6`/`d`/`;`→Other; `8` (telnet)→Other while `T` (tn3270)→Telnet, an inversion ([gopher.rs:112-119](../../../../errand/src/parse/gopher.rs)). The fetch/handling hint the type char exists to carry is flattened |
...
**Feed** ([errand/src/parse/feed.rs](../../../../errand/src/parse/feed.rs)) —
...
**Gemtext / spartan**
([errand/src/parse/gemtext.rs](../../../../errand/src/parse/gemtext.rs)): add
