...
**Genet dependency**: the moveBefore engine
...
step 4 spends *after* this lands; the forest-dom-specific genet capabilities (F1–F2 below)
are new and tracked here until/unless they earn a genet-doc home.
...
render from one `GenetMultiRunner`). What the forest unlocks is dragging a *scrolled,
...
`GenetMultiRunner` already exposes `dom(id) -> Option<DomHandle>` and `root(id) ->
...
## The work: three genet capabilities + one meerkat rewire

1. **RunnerTree mounts at a node** (genet / xilem-serval). `build`/`rebuild` attach under a
...
2. **Subtree layout at (root, viewport, sheet)** (genet-layout). Each window gets its own
...
3. **Mutation routing by root containment** (genet-scripted-dom). One dom means one mutation
   ...
   vocabulary genet already has, not fight it.
...
they don't, so the first move is a **genet-side spike, not more plan**:
...
would mean genet-layout needs a real per-root incremental session, a bigger lift to scope
before committing.

## Slices (genet-first, same-DPI before multi-DPI, green checkpoints)

- **F1 (genet).** Multi-runner owns the shared dom; `push_projection` mints a window-root;
...
- **F2 (genet-layout).** The F0 spike hardened into real per-root sessions + mutation
...
- **Subtree layout is the make-or-break** (F0). If genet-layout can't isolate per-root
...
- **Keep the public seam stable.** If `dom(id)`/`root(id)` hold, meerkat stays a render-path
  rewire. If the forest tempts a wider genet API change, re-justify it against that
  leverage.
