...
**Date**: 2026-06-02
**Status**: Plan for review. Phase 1 of the genet-as-host flip
([2026-06-01_genet_host_flip_plan.md](2026-06-01_genet_host_flip_plan.md), Phase 1).
The genet-side perf prerequisites (A/B/C + persistent Stylist) are done and
verified, so the per-frame motion mechanism is ready. This doc plans the orrery
element itself.
**Grounded in**: a read-only recon across mere + genet (platen producer, genet
element/paint extension points, gyre, the current-host orrery), plus a first-hand
read of genet's stacking-paint to settle the camera model.
...
- **genet has no custom-element / replaced-element / custom-paint hook.** The box
  tree is a closed set (block/flex/grid, `<img>`-only replaced, parley text). So
  "the orrery element" is **not** a new genet box type. It is a **host-side
  composition**: a plain genet `<div position:relative>` holding one abs-pos
...
transform**. The element owns: a genet container `<div>`, a gyre `Simulation`, the
`Camera`, and the per-frame loop.

```text
   container <div id=orrery-stage> with CSS transform = camera matrix
              │   (zoom + pan; genet propagates it to the subtree, incl. abs-pos)
   ┌──────────┴───────────────────────────────────────────┐
   ...
   │   one abs-pos genet subtree per visible node at its     │  child sits at its
   ...
   └─────────────────────────────────────────────────────────┘  transform applies
   composition seam: concat layer-1 commands + the document's GenetPaintList
...
the orrery container `<div>`, applied by genet to the whole subtree — the abs-pos
...
**This requires fixing genet's paint first.** Today genet's Tier-2 stacking
...
content via genet `FragmentQuery` and scene geometry via gyre, both in world space.

---

## Decisions (★ = decided 2026-06-02 with the architect)

- **★ D1 — Camera model: Option A (decided).** Fix genet's Tier-2 stacking so
  ...
  foundational build step 1A and improves genet paint beyond the orrery.
...
- **★ D3 — Scene hit-testing: edge-pick + marquee included (decided).** Build the
  ...
  through genet `FragmentQuery`. These are net-new features (no current-host
...
- **D4 — Composition seam.** Concatenate the underlay `CanvasPaintList` command
  stream with the document's `GenetPaintList` stream (remapping the font/image key
...
- **D7 — Navigation.** Author wheel=pan, ctrl+wheel=zoom, middle-click=pan, and
  inertia fresh on genet's input model (all net-new; the current host is
...
- **1A — genet: ancestor-transform propagation to abs-pos layers.** Fix
...
- **1B — Composition seam.** A `paint_list_render` (or host) helper that
  ...
  **Done**: a test composites an underlay stream + a genet document stream into one
...
- **1D — The orrery element (genet-hosted demo).** Compose the three layers in a
  pelt-live-shaped genet host: container div with the camera CSS transform (1A),
  ...
  **pre-materialized pool** (visibility toggled attribute-only → `RepaintOnly`),
  ...
  churn, and shows force + visual couplings, hosted by genet. (Built where it can
  be proven now; re-homing into mere's genet-host comes with the later flip phases.)
- **1E — Navigation + two-hit-test split.** wheel=pan / ctrl+wheel=zoom /
  ...
  genet `FragmentQuery`; scene geometry (empty-space, node, edge, marquee) via gyre
...
- **Rebuild against genet**: everything else in `graph_canvas.rs` (Masonry Widget
  ...
  abs-pos genet DOM children moved by inline transform under the container camera.
