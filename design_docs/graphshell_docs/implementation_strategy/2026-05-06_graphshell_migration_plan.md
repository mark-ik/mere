...
| `verso-tile` | Rendering surface identity, tile-slot placement, surface receiving/lifecycle between inker/platen/host | Engine selection, GraphWorkspace graph mutation, product chrome | Replace the temporary `inker::routing::SurfaceTargetId` bridge with narrower verso-tile surface contracts when the surface layer becomes real |
