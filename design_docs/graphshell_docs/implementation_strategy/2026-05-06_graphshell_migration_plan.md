...
| `platen` | Graph-aware composition/layout policy, frame/tile arrangement projection, layout constraints, renderable workbench model | Engine lifecycle, host widget handles, durable graph mutation | `platen::canvas_scene` and `platen::workbench` now own the extracted graph-to-canvas derivation core, frame/pane model, and initial active-frame/root-view selectors; Graphshell should keep shrinking toward wrappers around those selectors |
| `verso-tile` | Rendering surface identity, tile-slot placement, surface receiving/lifecycle between inker/platen/host | Engine selection, GraphWorkspace graph mutation, product chrome | `SurfaceTargetId`, `SurfaceHostId`, the `SurfaceEffect` / `SurfaceRequest` contract, and surface-effect constructors now live here; grow richer surface lifecycle/types here instead of pushing them back into `inker` or Graphshell |
...
- Moved the remaining surface-facing Graphshell seam into `verso_tile::surface`. `SurfaceHostId`, `SurfaceEffect`, and `SurfaceRequest` now live beside `SurfaceTargetId`, and `graphshell::app_state` only re-exports those types so the reducer/effect API stays stable while the surface contract belongs to the surface layer.
