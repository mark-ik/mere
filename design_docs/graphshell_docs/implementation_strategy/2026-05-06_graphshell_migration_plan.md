...
| `platen` | Graph-aware composition/layout policy, frame/tile arrangement projection, layout constraints, renderable workbench model | Engine lifecycle, host widget handles, durable graph mutation | `platen::canvas_scene` now owns the first extracted graph-to-canvas derivation core; Graphshell should shrink toward view/workspace wrappers rather than re-accumulate composition logic |
| `verso-tile` | Rendering surface identity, tile-slot placement, surface receiving/lifecycle between inker/platen/host | Engine selection, GraphWorkspace graph mutation, product chrome | `SurfaceTargetId` now lives here as the first real surface contract; grow richer surface lifecycle/types here instead of pushing them back into `inker` or Graphshell |
