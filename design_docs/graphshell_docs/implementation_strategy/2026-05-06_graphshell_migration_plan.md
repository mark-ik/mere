...
- Next app-state reducer target: keep moving sequentially into the remaining pure `composition` and `app_ux` seams without moving `GraphBrowserApp`, persistence implementations, or host adapters.
