...
| `app/runtime_lifecycle.rs` webview-node maps | `map_webview_to_node`, `unmap_webview`, `get_node_for_webview`, `get_webview_for_node`, `webview_node_mappings` | Host adapter / Verso-Tile lifecycle | Do not place renderer IDs in `GraphWorkspace`. Active surface placement now carries `NodeKey` through `verso_tile::surface::SurfaceSlotPlacement`, while `graphshell_runtime::surface_schedule::apply_viewer_surface_schedule` applies present/retire schedules through `ViewerSurfaceHost`; concrete hosts still own renderer/webview IDs. |
| `crates/verso-host/src/viewer_surface_host.rs` | `ViewerSurfaceRegistryHost`, `ServoViewerSurfaceHost`, `NoopViewerSurfaceHost` | Host adapter | Migrate the registry-backed allocation pattern, not the old Servo/Verso naming. `graphshell-host` now owns `ViewerSurfaceRegistryHost` and `SurfaceFactoryHost`; the no-op host, old `verso` route binding, Tokio spawner, and renderer IDs are not imported into Mere. |
