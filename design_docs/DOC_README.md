...
- [host_cheap_path_plan](mere_docs/implementation_strategy/2026-06-10_host_cheap_path_plan.md) — move meerkat's DOM panes off the stateless per-frame pipeline onto IncrementalLayout sessions + a laid-out-document query seam; stop the mutation-log leak. *(C0–C5 + C4c DONE: chrome on a session, frame -40%; chrome queries + a11y tree + all panes single-layout. C6 wiring parity spun out → host_wiring_grabbag_plan. Ready to archive.)*
- [host_wiring_grabbag_plan](mere_docs/implementation_strategy/2026-06-11_host_wiring_grabbag_plan.md) — the host cheap-path plan's spun-out C6: eight remaining serval/xilem-serval host-wiring items in two phases. G1 composition-runway (on_wheel view, transform-aware hit-test, pointer cancellation, memoize) feeds window_composition P2+; G2 host-completeness (IME, environment threading, keyboard escape hatches, chrome a11y actions). Most are wired one layer down with no meerkat caller.
...
- [multi_graph_activation_plan](mere_docs/implementation_strategy/2026-06-09_multi_graph_activation_plan.md) — wire meerkat to hold many graphs per window, switchable from the shellbar (Model B = window holds panes/graph is content, goal; Model A = session owns the content band, checkpoint at MG2). MG1–MG5 + host text path done (per-session storage, switch/close/rename, window-scoped frame, labelled switcher tiles); MG6 (far-B leaf coexistence) remains.
...
- [omnibar_command_shell_plan](mere_docs/implementation_strategy/2026-06-13_omnibar_command_shell_plan.md) — **planned**: the omnibar as a programmable command line over the `Command` spine. A `>`-sigil arm in `nav::classify` routes to a privileged `CommandShell` (meerkat-side rhai engine) that evals against a read-only `ShellContext` snapshot and emits `Vec<Command>`, drained through the existing `InvokeCommand` run path. Two-tier trust (sandboxed knot-note lane vs privileged omnibar lane), one language, privilege = the binding set. S4 reaches across to orrery graph/tile queries (converges with the inspector `ContentReport`).
