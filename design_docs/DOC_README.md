...
4. Per-area roots (populated as work proceeds):
   - `mere_docs/` — product-level concerns (Navigator, scope spectrum, UX, cross-cutting protocol architecture)
     ...
     - [`mere_docs/research/2026-05-08_local_intelligence_integration_research.md`](mere_docs/research/2026-05-08_local_intelligence_integration_research.md) — Burn-first local AI stack (graphshell-era research updated for Mere); intelligence taxonomy + capability tiers + Distillery contract carry forward; first-cut slice is embeddings via Burn + vector index in eidetic + field-algebra `Sample` integration for force-directed-by-meaning; tier-3+ deferred behind Distillation Boundary / AWAL prerequisites; browser/PWA viable for embeddings, native-only for LLMs
