...
- [window_composition_plan](mere_docs/implementation_strategy/2026-06-11_window_composition_plan.md) — the second-window architecture as a two-axis `(content, linkage)` model: orrery vs orrery-less windows (workbench tile / gloss), independent vs primary-secondary linkage, and a general cross-graph move/copy with provenance. Moves the orrery off `Shell` into the window (P1, enabled by the UUID-keyed shared constellation, no registry for different graphs); supersedes multi_window MW4–MW6. Same-graph-in-two-windows registry stays deferred.
