...
| DDA traversal | Product-free traversal kernel/data contract, final crate selected by adoption | Product lens owns camera, material, and composition | Mesocosm and Paredros headed profiles share WGSL/brick ABI in receipts | Permanent tracked adoption by the second product | Owner selected 2026-08-26: `conatus-brick` (branch `codex/conatus-brick-lift`, pinned at `bd8f0044` by both consumers); joins main when the branch lands |
