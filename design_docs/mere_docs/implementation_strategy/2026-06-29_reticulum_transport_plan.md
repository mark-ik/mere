...
### Phase 0 — API and dependency verification (complete)

Done when:
- `cargo check -p transport --features reticulum` passes.
- Exact `reticulum` APIs for identity, destination, announce, link, and packet
  events are documented in Findings.
- Identity derivation strategy is chosen.

### Phase 1 — Core transport skeleton
...
- `Transport` trait is implemented for `ReticulumTransport`.
...
- Unregistered-ALPN, peer-id consistency, deterministic identity, and binding
