...
One product-neutral Graphshell process discovers and mounts Turnstone and
...
- Turnstone: one browsing-graph session with three items and two routed
  relations;
...
- rejected Turnstone and Isometry product actions with endpoint-owned reasons.
...
Clippy gate pass. Turnstone's three focused endpoint tests and endpoint-binary
build pass; its wider dependency graph retains existing warnings.

## Dependency direction

`cargo tree -p graphshell` contains neither Turnstone nor Isometry. Product
