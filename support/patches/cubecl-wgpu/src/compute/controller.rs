...
impl WgpuAllocController {
    ...
    pub fn init(binding: ManagedMemoryBinding, buffer: wgpu::Buffer) -> Self {
        ...
        // wgpu 30 made this fallible. The buffer is mapped by the caller
        // before `init`, so a failure here is a broken invariant, not a
        // runtime condition, and this constructor has no error channel.
        let buf_view = buffer
            .get_mapped_range(..)
            .expect("buffer was not mapped before ManagedBufferController::init");
