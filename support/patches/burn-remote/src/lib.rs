...
mod tests {
    ...
    /// A host can inspect one admitted Iroh session, close only that session,
    /// and wait until the client observes termination without dropping the
    /// application-owned endpoint or router.
    #[test]
    #[cfg(not(feature = "fusion"))]
    pub fn test_targeted_iroh_session_close() {
        use crate::{
        ...
        let session = rt.block_on(async {
            tokio::time::timeout(std::time::Duration::from_secs(5), async {
                loop {
                    if let Some(session) = control.sessions().await.into_iter().next() {
                        break session;
                    }
            ...
            .await
            .expect("admitted session becomes visible")
        });
